...
function updateDBEntryUp(obj){
    readFromDBFile(function(data){
         ...
         * */
        for(var i = 0; i < dataParsed.length;i++){
            if(dataParsed[i].user === obj.user && dataParsed[i].url === obj.url){ //find message on which to vote up
                if(dataParsed[i]['positive'].indexOf(obj.currentUser) > -1){
                     ...
                     * */
                    if(dataParsed[i]['negative'].indexOf(obj.currentUser) > -1){
                        for(var j = 0; j < dataParsed[i]['negative'].length;j++){
                            if(dataParsed[i]['negative'][j] == obj.currentUser){
                                dataParsed[i]['negative'].splice(j,1);
                            }
                        }
                    }else{
                        dataParsed[i]['positive'].push(obj.currentUser);
                    }
                ...
                if(dataParsed[i]['negative'].indexOf(obj.currentUser) > -1){
                    ...
                    if(dataParsed[i]['positive'].indexOf(obj.currentUser) > -1){
                        for(var j = 0; j < dataParsed[i]['positive'].length;j++){
                            if(dataParsed[i]['positive'][j] == obj.currentUser){
                                dataParsed[i]['positive'].splice(j,1);
                            }
                        }
                    }else{
                        dataParsed[i]['negative'].push(obj.currentUser);
                    }
